...
export default function CreativePage() {
  return (
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 sm:py-16">
      <section className="overflow-hidden rounded-[2rem] border border-gray-200/80 bg-gradient-to-br from-white/72 via-white/64 to-fuchsia-50/52 p-7 shadow-sm backdrop-blur-xl dark:border-gray-800/80 dark:from-gray-950/72 dark:via-gray-950/64 dark:to-fuchsia-950/25 sm:p-12">
      ...
      <section className="mt-8 grid gap-4 sm:grid-cols-3">
        {stages.map(({ label, detail, icon: Icon }, index) => <div key={label} className="rounded-3xl border border-gray-200/80 bg-gray-50/48 p-6 backdrop-blur-md dark:border-gray-800/80 dark:bg-gray-900/35"><div className="flex items-start justify-between"><Icon className="text-fuchsia-600 dark:text-fuchsia-400" size={24} /><span className="font-mono text-sm text-gray-400">0{index + 1}</span></div><p className="mt-8 text-xs font-bold uppercase tracking-[0.2em] text-gray-400">{label}</p><h2 className="mt-2 text-xl font-bold text-gray-900 dark:text-white">{detail}</h2></div>)}
      </section>
